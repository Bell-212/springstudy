...
<div>
  ...
  <div>
    <form id="frm_comment_add">
      ...
      <input type="hidden" name="blogNo" value="${blog.blogNo}">
      <button type="button" id="btn_comment_add" class="btn btn-secondary btn-sm">작성완료</button>
    </form>
  </div>
  ...
  <script>
	...
	const fnCommentList = ()=>{
		$.ajax({
			...
			// 응답
			dataType: 'json',
			success: (resData) => { // resData = {"commentList": [], "paging": "<div>...."}
				$('#comment_list').empty();
				$('#paging').empty();
					...
					if(c.status === 0){
					  str += '<div>삭제된 댓글입니다.</div>';
						...
						if('${sessionScope.user.userNo}' == c.userDto.userNo){
        					...
        					str += '  <i class="fa-regular fa-circle-xmark btn_remove_comment"></i>';
        					str += ' </div>';
						}
					}
					str += '</div>';
	...
	답글
		...
		<div>
			<input type="hidden" value="'+ c.commentNo +'">
