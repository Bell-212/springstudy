...
public class UserServiceImpl implements UserService {
  ...
  private final String client_id = "rjH76EEaARpAfAoMykHW";
  private final String client_secret = "rnU18PRe8Z";
  ...
  public void login(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    if(inactiveUser != null) {
      // 정보저장(inactiveUser) 리다이렉트의 경우 세션에 정보 저장하는게 용이함
  ...
  public String getNaverLoginAccessToken(HttpServletRequest request) throws Exception {
    ...
    // 네이버로그인-1 의 응답 결과(access_token을 얻기 위해 요청 파라미터로 사용해야함)
  ...
  @Override
  public void inactiveUserBatch() {  // 스케쥴러가 부른다.
  ...
  public void active(HttpSession session, HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      if(insertActiveUserResult == 1 && deleteInactiveUserResult == 1) {
        out.println("alert('휴면계정이 복구되었습니다. 계정활성화를 위해 곧바로 로그인 해 주세요.')");
        out.println("location.href='" + request.getContextPath() + "/main.do'");  
        // 로그인페이지로 보내면 로그인 후 다시 휴면 계정복구 페이지로 돌아오므로 main으로 이동한다.
      } else {
        out.println("alert('휴면계정이 복구가 실패했습니다. 다시 시도해주세요.')");
        out.println("history.back()");  
      }
      out.println("</script>");
      out.flush();
