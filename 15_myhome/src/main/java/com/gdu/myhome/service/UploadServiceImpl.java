...
public class UploadServiceImpl implements UploadService {
  ...
  public boolean addUpload(MultipartHttpServletRequest multipartRequest) throws Exception {
    ...
    int UploadCount = uploadMapper.insertUpload(upload); // 실행후 uploadNo가 저장됨(uploadMapper.insertUpload의 selectkey동작)
    
    // 스프링에서 첨부된 파일들은 MultipartFile타입이다.
    ...
    for(MultipartFile multipartFile : files) {
      if(multipartFile != null && !multipartFile.isEmpty()) {
        ...
        if(hasThumbnail == 1) {
          ...
          Thumbnails.of(file)           // thumbnailator 라이브러리 사용
  ...
  public Map<String, Object> getUploadList(HttpServletRequest request) {
    ...
    myPageUtils.setPaging(page, total, display); // begin, end 구함
