...
CREATE TABLE LEAVE_USER_T (
    EMAIL VARCHAR2(100 BYTE) NOT NULL UNIQUE,  -- 탈퇴한 사용자 이메일
    JOINED_AT DATE,                            -- 가입일         
    LEAVED_AT DATE                             -- 탈퇴일
);
...
-- SELECT STANDARD_HASH('1111', 'SHA256') FROM DUAL;


-- 쿼리 테스트

-- 1. 로그인 할 때(EMAIL, PW 입력)
-- 1) 정상 회원
SELECT USER_NO, U.EMAIL, PW, NAME, GENDER, MOBILE, POSTCODE, ROAD_ADDRESS, JIBUN_ADDRESS, DETAIL_ADDRESS, AGREE, PW_MODIFIED_AT, JOINED_AT
  FROM USER_T
...
-- 3. 휴면 처리 할 때(12개월 이상 로그인 이력이 없다. 로그인 이력이 전혀 없는 사용자 중 가입일이 12개월 이상 지남)
